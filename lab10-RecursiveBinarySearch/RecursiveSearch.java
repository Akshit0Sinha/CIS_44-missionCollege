...
public class RecursiveSearch {
	...
	public static int recursiveBinarySearch(int[] arr, int key, int low, int high) {
		...
		// Step 2: Check if middle element is the key
		if (arr[mid] == key) {
			return mid;
		}
		// Step 3: If key is smaller, search the left subarray
		else if (key < arr[mid]) {
			return recursiveBinarySearch(arr, key, low, mid - 1);
		}
		// Step 4: If key is larger, search the right subarray
		else {
	...
	public Node searchBST(Node root, int key) {
		...
		// Base Case 1: Tree is empty or key is found at root
		if (root == null || root.key == key) {
			return root;
		}

		// If the key is less than the root's key, make a recursive call 
		//    on the left subtree.
		...
		// If the key is greater than the root's key, make a recursive call 
